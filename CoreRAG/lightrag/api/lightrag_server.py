...
def create_app(args):
    ...
    async def get_embedding_func(args):
        """根据配置返回适当的嵌入函数"""
        if args.embedding_binding == "lollms":
        ...
        elif args.embedding_binding == "QWEN":
            from custom_rag_processor import CustomRAGProcessor
            return lambda texts: CustomRAGProcessor.qwen_embed_wrapper(texts, args.embedding_binding_api_key)
        else:  # openai
    ...
    async def get_completion_func(args):
        """根据配置返回适当的完成函数"""
        if args.llm_binding == "lollms":
            ...
            from custom_rag_processor import CustomRAGProcessor
            return lambda prompt, **kwargs: CustomRAGProcessor.deepseek_complete_wrapper(
                prompt, args.llm_binding_api_key, **kwargs
            )
        else:  # openai or openai-ollama
            from lightrag.llm.openai import openai_complete_if_cache
    ...
    embedding_func = EmbeddingFunc(
        embedding_dim=args.embedding_dim,
        max_token_size=args.max_embed_tokens,
        func= await get_embedding_func(args),
    )

    completion_func = await get_completion_func(args)   
    ...
    async def login(form_data: OAuth2PasswordRequestForm = Depends()):
        ...
        user_token = auth_handler.create_token(
            username=username, role="user", metadata={"auth_mode": "disabled"}
        )
        return {
