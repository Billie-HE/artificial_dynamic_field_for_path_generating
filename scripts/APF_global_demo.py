...
class Visualizer:
    ...
    def show(self, Z, agent_pos, t, start, goal):
        self.im.set_clim(np.min(Z), np.max(Z))
        self.im.set_data(Z.T)
        ...
        self._artists.append(self.ax.plot(agent_pos[0], agent_pos[1], 'ro', label='Agent')[0])
        self._artists.append(self.ax.plot(start[0], start[1], 'bs', label='Start')[0])
        self._artists.append(self.ax.plot(goal[0], goal[1], 'g*', label='Goal')[0])

        if len(self.full_path_ref) > 1:
            xs, ys = zip(*self.full_path_ref)
            self._artists.append(self.ax.plot(xs, ys, 'w.-', alpha=0.8, label='Full Path')[0])

        # 再次设置范围和比例，防止动态更新导致偏移
        self.ax.set_xlim(-0.5, self.grid_size[0] - 0.5)
        self.ax.set_ylim(-0.5, self.grid_size[1] - 0.5)
        self.ax.set_aspect('equal')
...
def plan_segment(start, goal, agent_path_accum, vis):
    ...
    for t in range(TOTAL_STEPS + 1):
        ...
        agent.step(Z)
        vis.show(Z, agent.pos, t, start, goal)  # ← 传入起止点用于动态标签显示

        # if np.array_equal(agent.pos, goal):
        #     break

    agent_path_accum.extend(agent.path[1:])  # ← 追加路径点（跳过起点防重复）
